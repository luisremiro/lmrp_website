...
bibtex_2academic <- function(bibfile,
  ...
  pacman::p_load(RefManageR, dplyr, stringr, anytime, tidyr, stringi, bib2df)
  
  # Import the bibtex file and convert to data.frame
  mypubs <- bib2df(bibfile) %>%
    as.data.frame()
  
  names(mypubs) <- tolower(names(mypubs))
  ...
  create_md <- function(x) {
    ...
    if (!file.exists(file.path(outfold, filename)) | overwrite) {
      ...
      if (!is.na(x[["annote"]])) write(paste0(x[["annote"]]), fileConn, append = T)
    }
  }
